...
| `model_tier` | `cli` · `haiku` · `sonnet` · `opus` · `fable` (Codex aliases accepted in budget lines: `luna` · `terra` · `sol`) | deterministic → CLI (no model); bounded high-fanout judgment (extract/classify/label/summarize; never code edits/reviews) → Haiku (opt-in utility rung below fleet, chosen up-front by task classification — outside the promotion/demotion ladder); judgment → Sonnet/Luna (fleet, default); escalation/synth → Opus/Terra (one escalation slot + one synthesis slot max per pipeline); conductor-only → Fable/Sol (at most one dispatched unblock call after Opus/Terra failed; justify per-slot). Escalate to Opus/Terra only on a demonstrated fleet shortfall, never precautionarily. (canon: `project-meta/references/multi-agent-protocols.md#model-tier`) |
