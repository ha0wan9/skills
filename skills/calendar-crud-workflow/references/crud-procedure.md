# Calendar CRUD Procedure

Full 8-step workflow for calendar event CRUD operations, plus classification model and description contract.
...
- Project calendars such as `Neuromatch`: recurring program-specific schedules needing stable color and on/off control

Do not create a calendar for every small topic — use description tags instead.

## Prefix Rules

**State prefixes** express transient workflow status and are stripped once resolved:

- `[Hold]`: placeholder, not confirmed
- `[Tentative]`: time or commitment uncertain
- `[Prep]`: preparation block

**Type prefixes** mark a durable category for at-a-glance triage (`[Admin]`, `[Paper]`, `[NMA]`). Use a type prefix only when the event is not already on a type-specific calendar AND calendar color alone is insufficient for scanning in agenda/month view. Otherwise express the type as a description tag and omit the prefix.
...
Use lowercase kebab-case tags. Keep source links and meeting links in separate fields.
...
For batch operations, show a compact proposal before writing:
...
- Batch delete: list the exact events first, require confirmation.
- Recurring event: require scope clarity (this instance / this-and-following / entire series) before deleting.
