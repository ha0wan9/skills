...
Shared infra inside `project-meta`. Consumed by `audit`/`deliver`, by the roadmap co-review transaction (DASH-08), and — later — by the orchestration skill via the root-skill pointer pattern (it is **not** a separate skill). It sits between **AP-COORD-2** (you MUST review) and **AP-COORD-4** (don't over-orchestrate): every review is fast, and tokens are proportionate to stakes.
