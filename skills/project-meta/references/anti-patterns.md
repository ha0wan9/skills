...
(Sonnet/Luna), fails, gets retried on the escalation tier (Opus/Terra),
and ships — then next session repeats the exact sequence; or
every task-type has silently crept to (Opus/Terra, max) and no one can say which
...
expensive one (model); cap at (Opus/Terra, max); record the *cause* so a stale
