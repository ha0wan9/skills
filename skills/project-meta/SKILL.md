...
- Coordination: the user explicitly asks for multi-agent work, or task complexity warrants planning, delegated execution, and review.
...
3. Read canonical project memory when it exists; it routes what to load next. When it points to `agents/*.md` or another topical-memory directory, load only files relevant to the task.
4. Inspect shared docs lightly (title, first section, TOC, heading map); read fully only when short or directly relevant.
...
- Shared docs are the primary project explanation but read them selectively; keep a README structure map in agent-facing docs once a shared doc grows structurally important.
...
- Treat repeated agent mistakes as missing harness — improve docs, routing, validation, or tooling, not just the output; keep the harness agent-legible (concise map, versioned truth, selective loading, verifiable rules).
...
| Debug / root-cause a hard, flaky, or recurring bug | **`meta-debug`** | provides the harness it reads/writes (memory layout, dispatch protocol, lesson CRUD); init first if none |
...
| Intra-task software-engineering *method* (brainstorm→plan→TDD→review→verify) | **the methodology plugin** (external, not bindable) | defer intra-task process; own harness/skill authoring + cross-task governance; be additive |

If an arbitration is unclear, ask the user before invoking either skill. Never silently invoke both.

## Gotchas

- **`USER.md` is local-only and Git-ignored** — never stage or commit it; merge the skill's `.gitignore.template` into the target repo before any `USER.md` exists.
...
On `/project-meta <command>`, load exactly **one** recipe; it owns that verb's workflow end-to-end (trigger / mode / required references / steps / output / anti-patterns):
...
Cross-cutting policy (route contract, reserved verbs, shared rules, footer contract) lives in [cli-command-patterns](references/cli-command-patterns.md). A non-command trigger loads no recipe — resolve arbitration, then load the single matching reference below.
...
- Should the lesson remain documentation, or become a linter, script, template, checklist, or recurring cleanup routine?
- Has the pre-commit delivery been shown to the user when a commit will be created?
- If the note passes, update the relevant canonical memory file; if not, leave memory untouched.
