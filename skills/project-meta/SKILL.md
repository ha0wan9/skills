---
name: project-meta
description: "Bootstrap, audit, and evolve a repository agent-work harness via /project-meta commands (init, status, validate, deliver, audit): canonical memory (AGENTS.md) and local USER.md preferences, agent- and user-facing docs with pre-commit delivery, templates instantiated into project artifacts, behavior guardrails and execution policy, multi-agent dispatch with review, mirror sync, the Claude Code hooks pack, multi-host manifests, phase-lock gates, a skill-critic suite, and pressure-testing of MUST-rules. Use when starting work in a repo, creating or repairing repo memory, improving agent instructions, coordinating complex project work, authoring or auditing a skill, or turning validated lessons into durable harness updates."
---
