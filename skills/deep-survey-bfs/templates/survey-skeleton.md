...
> **Provenance note (required).** State the dominant `source_tier` of the
> backing claims (see `claims.jsonl`): were findings read from full-text PDFs,
> or from abstracts / bibliographic metadata / search results? If the survey
> was built from abstracts+metadata rather than full PDFs, say so plainly and
> note that `claims.jsonl` quotes are finding-level (not certified verbatim),
> carry `confidence` values, and that verbatim accuracy against primary
> sources remains a reader responsibility. This keeps the survey's abstraction
> level honest by default — omitting it implies full-text verification that may
> not have happened.
