...
| `global-meta` | [`skills/global-meta/`](skills/global-meta/) | Bootstrap, audit, and evolve a user's **global** Claude Code / Codex config root (`~/.claude`, `~/.codex`, and `~/.claude-<name>` / `~/.codex-<name>` profiles). Replaces the retired `profile-creator`. Live: `create` (profile/launcher), `status` (read-only inventory), `audit` (four-way consistency findings), `snapshot`/`restore` (store ledger); roadmap: drift/reconcile/settings/track (see `skills/project-meta/proposals/global-meta.md`). |
