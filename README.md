...
| `global-meta` | [`skills/global-meta/`](skills/global-meta/) | Bootstrap, audit, and evolve a user's **global** Claude Code / Codex config root (`~/.claude`, `~/.codex`, and `~/.claude-<name>` / `~/.codex-<name>` profiles). Replaces the retired `profile-creator`. Live: `create` (profile/launcher), `status` (read-only inventory), `audit` (four-way consistency findings), `audit --emit-fix` (reviewable bash remediation script; dry-run by default, `--apply` to execute, snapshots before first mutation), `snapshot`/`restore` (store ledger); roadmap: drift/reconcile/settings/track (see `skills/project-meta/proposals/global-meta.md`). |
...
| `global-meta` | ✅ | ✅ | ❌ | Claude Marketplace |
...
└── skills/                        # flat skill directory, Anthropic-pattern
    ...
    │   ├── examples/                   # sample-profile/, audit-fixture/, audit-fixture-clean/
    │   └── scripts/                    # create_profile.py, config_root_audit.py (status/audit/snapshot/restore)
    ├── dl-research/
    │   ├── SKILL.md
    │   ├── agents/openai.yaml
    │   ├── modes/
