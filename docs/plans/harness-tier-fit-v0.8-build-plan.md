...
| A-2 | Phase-lock **hard** gates under `standard` profile provide enforcement margin a Fable-class session no longer needs (advisory suffices; `strict` keeps hard) | inferred | WORKING | high | Prior evidence: A-1 doctrine (official de-prescription guidance). Repo-specific confirmation pending: DASH-083a A/B note + 🔴 operator evidence gate — **no gate-authority edit before the gate clears** |
