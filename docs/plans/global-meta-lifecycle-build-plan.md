...
readiness: strict
goal: "Ship the v0.5 milestone: global-meta gains read-only status+audit (config_root_audit.py + snapshot ledger) and audit --emit-fix; the marketplace slimming question gets a sandboxed GO/NO-GO verdict; the scope-aware reload lands; the proposal records the verdict. DASH-036 (slimming implementation) is checkpoint-gated and NOT built in this run."
---

# global-meta lifecycle (v0.5) — Build Plan

**Milestone:** v0.5 "global-meta lifecycle + marketplace hygiene" — DASH-033/034/035/036/037/038
...
**Done =** DASH-033, -034, -035, -037, -038 land on main with green gates; DASH-036 has a
...
- NO marketplace slimming *implementation* this run (DASH-036 is 🔴-gated).
...
| Item | Tier | Why |
|---|---|---|
| DASH-037 scope-aware reload | 🟢 autonomous — **DONE** (PR #43 landed 2026-06-11, main e4bd7c3) | chip-authored diff; gates were mechanical |
| DASH-038 proposal write-back | 🟢 autonomous | docs-only; collision check is the gate |
| DASH-033 status+audit | 🟡 fixture-bound | built against the committed seeded fixture (§4 F1–F4) |
| DASH-034 emit-fix | 🟡 fixture-bound | converge-the-fixture-copy loop is the acceptance |
| DASH-035 slimming spike | 🟡 autonomous-against-a-specced-fixture (runtime-generated per §4, not committed) | sandboxed `CLAUDE_CONFIG_DIR`; read-only intent on the real root — see §7.2 caveat |
| DASH-036 slimming impl | 🔴 checkpoint | distribution-semantics change; needs spike GO + operator approval |
...
- **F-audit (COMMITTED):** `skills/global-meta/examples/audit-fixture/` (seeded findings
  ...
  `--spec-marketplace test-mkt`). `config_root_audit.py` itself is DASH-033's deliverable —
...
1. **Wave 0 (Lead, inline) — DONE 2026-06-11:** DASH-037 shipped as PR #43 (chip-authored
   branch `ship-reload-scope-aware`, fresh review APPROVE, landed; main `e4bd7c3`).
2. **Wave 1 (parallel, Sonnet workers):** DASH-033 (worktree; owns ALL `skills/global-meta/` edits
   incl. its arbitration row) ∥ DASH-035 (read-only spike, no repo edits) ∥ DASH-038 (worktree;
...
- Version bumps: DASH-033 → global-meta **1.1.0**; DASH-034 → **1.2.0**; DASH-037 already carries marketplace 2.0.2; DASH-038 → project-meta patch + meta-debug patch.
...
- Spike verdict storage: board item body (DASH-035) + §9 of this plan; no new doc file.
...
- 2026-06-11 — **Round 1** (fresh-context auditor, Goal-readiness dimension): **NO-GO**.
  Blockers: F-audit fixtures absent/circular; DASH-034 §6 test target was an ellipsis.
  Majors: DASH-037 premise stale (PR #43 was already open; landed during the fix window);
  §9 unfilled; `agents/project-artifacts.md` missing. Minors: F-spike schema underspecified,
  DASH-038 pre-state note, sandbox fence not asserted in §6, post-merge rerun commands
...
- 2026-06-11 — **Fixes applied** (this revision): fixtures committed (`audit-fixture{,-clean}/`),
  manifest created, §§2/3/4/5/6/8 amended as listed above; DASH-037 marked done (PR #43 merged).
...
- 2026-06-11 — **DASH-035 spike verdict: GO.** Scoped sources work natively
  ...
  DASH-036 remains 🔴 — awaiting operator approval.
